...
from setup_databases import (
    ...
    create_caida_as_table,
    create_cf_servers_table,
    create_cf_tables,
    create_cities_table,
    create_countries_w_starlink_measurements_table,
    create_ndt7_table,
    create_ndt_servers_table,
...
def populate_cf_tables(
    conn: connection,
    csv_names: Dict[str, str] = {
        "mean": "cf-flat-mean.csv",
        "median": "cf-flat-median.csv",
        "90th_percentile": "cf-flat-90th.csv",
    },
) -> None:
...
def populate_ndt_servers_table(
    ...
    csv_path = data_dir / csv_name
    insert_data_from_csv(
        conn, csv_path, ndt_best_server_insert_query, clean_dataframe=_clean_servers
    )
...
def populate_countries_w_starlink_measurements_table(
    conn: connection, csv_name: str = "countries_w_starlink_measurements.csv"
) -> None:
...
def populate_cities_table(
    ...
    admin1_df[["admin1_country", "admin1_code"]] = admin1_df["admin1_full_code"].str.split(
        ".", expand=True
    )
